<tr class="top aligned field-meta-description">
	<th class="collapsing">
		{{ $field_title }}
	...
	<td>
		<div class="field ui right labeled input" data-name="field[{{ $id_language }}][{{ $field['name'] }}]">
			<input type="text" name="field[{{ $id_language }}][{{ $field['name'] }}]" value="{{ $field['value'] }}"
				...
				class="{{ array_key_exists('required', $field) && $field['required'] === true ? 
					'validation-required' : '' }}" />
			<div class="character-count ui basic label">160</div>
